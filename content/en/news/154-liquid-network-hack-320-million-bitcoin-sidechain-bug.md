...
What makes this episode even stranger is the withdrawing party's own behavior. The individual or group behind it has been sending on-chain messages describing themselves as "white hat" actors and has reportedly offered to return the funds once the underlying bug is patched. On September 7, after Blockstream confirmed the underlying bug had been patched, the attacker made good on that offer - returning about 3,400 BTC, roughly 85% of what was taken, to Liquid Network's federation wallet. The remaining roughly 598.5 BTC (worth about $47 million, or about 15% of the total) stayed in the attacker's own wallet rather than being returned, effectively taken as a self-appointed "fee." That's a notable departure from how most malicious exploits play out - typically involving rapid laundering or cash-out attempts - and the industry is now watching whether that retained amount moves further and how Blockstream responds to it.
...
- **Headline severity and market impact don't always move in lockstep or on the same timeline.** Here, most of the funds were back within a day of the patch being confirmed, so the final loss settled out far smaller than the initial $320 million headline implied. Track how a situation develops - not just the first alarming number - before drawing conclusions.
...
### Did the hacker actually return the funds?
Mostly, yes. After Blockstream confirmed on September 7 that the vulnerability had been patched, the attacker returned about 3,400 BTC - roughly 85% of what was taken - to Liquid Network. The remaining approximately 598.5 BTC (about $47 million, roughly 15% of the total) was kept in the attacker's own wallet rather than returned, and it isn't yet clear whether that amount was a pre-agreed bounty or simply retained unilaterally.
