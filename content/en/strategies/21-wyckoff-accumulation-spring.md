...
title: "Wyckoff Method: Reading the Accumulation Schematic and Spring Pattern"
description: "Learn Wyckoff's five-phase accumulation schematic and Spring pattern to spot when large operators are quietly building a position before markup."
order: 21
updated: 2026-08-12
...
1. **Law of Supply and Demand**: price rises when demand overwhelms supply and falls when supply overwhelms demand. Obvious on its face, but Wyckoff's point was that this should be read through the *relationship* between price and volume, not price alone.
2. **Law of Cause and Effect**: the size of a subsequent move (the "effect") is proportional to how long and how wide the preceding accumulation or distribution (the "cause") was. A breakout out of a narrow, brief range tends to travel less far than one out of a wide range that took months to build.
3. **Law of Effort vs. Result**: volume ("effort") and the resulting price change ("result") should move in proportion. Heavy volume with little price progress is a mismatch — read as evidence that a large operator is absorbing supply on the other side.
...
<figure class="diagram">
  <img src="/static/img/charts/en/wyckoff-accumulation-spring.svg" alt="Diagram of the Wyckoff accumulation schematic Phases A through E: a downtrend halting at Preliminary Support and a Selling Climax, followed by an Automatic Rally and Secondary Test, a trading range building in Phase B, a Spring briefly piercing range support in Phase C, a Sign of Strength breakout on rising volume with a Last Point of Support pullback in Phase D, and Phase E markup trending price out of the range, with volume shown expanding on the Selling Climax and Sign of Strength and contracting during the Spring" loading="lazy">
  <figcaption>Phase A (downtrend stalls) → Phase B (cause is built) → Phase C (Spring shakes out the last sellers) → Phase D (SOS breakout + LPS) → Phase E (markup). Volume below expands sharply on the Selling Climax and Sign of Strength, but contracts during the Spring itself.</figcaption>
</figure>
...
Entering just after the Spring, at $9.15, and placing a stop per Lesson 6's [Risk-Reward and Money Management](/en/strategies/risk-reward-money-management/) framework slightly below the Spring low — say $8.65 — puts risk at roughly $0.50 per share. That produces a favorable risk-reward if the LPS-to-markup move is used as a target, but that ratio belongs to this one hypothetical scenario, not a general statistic for how Springs perform.
